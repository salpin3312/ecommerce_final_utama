...
function Navbar() {
   ...
   useEffect(() => {
      if (isAuthenticated && user?.role === "USER") {
         ...
         // Reset cart count when user is not authenticated
         setCartItemCount(0);
      }
   ...
   const fetchCartCount = async () => {
      try {
         // Only fetch if user is authenticated and is a regular user
         if (!isAuthenticated || user?.role !== "USER") {
            setCartItemCount(0);
            return;
         }
               ...
               <div className="navbar-start">
                  <div className="dropdown">
                     ...
                     <ul
                        ...
                        {navLinks.map((link) => (
                           <li key={link.to}>
                              <Link to={link.to} className={`text-base ${isActive(link.to) ? "font-bold" : ""}`}>
                                 {link.label}
                              </Link>
                           </li>
                        ))}
                        ...
                        {user.role === "USER" && (
                           <Link to="/cart" className="btn btn-ghost btn-circle mr-2">
                              <div className="indicator">
                                 ...
                                 {cartItemCount > 0 && (
                                    <span className="indicator-item badge badge-secondary">{cartItemCount}</span>
                                 )}
                              </div>
                           </Link>
                        )}
                        <div className="dropdown dropdown-end">
                           <label tabIndex={0} className="btn btn-ghost btn-circle avatar">
                              <div className="w-10 rounded-full">
                                 ...
                                 <div
                                    className={`w-full h-full rounded-full bg-primary text-primary-content flex items-center justify-center ${
                                       user.avatar ? "hidden" : ""
                                    }`}>
                           ...
                           <ul
                              ...
                              <li className="menu-title px-4 py-2">
                                 ...
                                 <span className="text-xs block opacity-70">{user.email}</span>
                              </li>
                              {user.role === "ADMIN" && (
                                 <li>
