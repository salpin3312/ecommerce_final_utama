...
// Konfirmasi penerimaan order (user)
export const confirmOrder = async (id) => {
   try {
      const response = await axiosInstance.put(`/api/orders/confirm/${id}`);
      return response.data;
   } catch (error) {
      throw error;
   }
...
// Buat/ubah ulasan untuk order
export const upsertReview = async (orderId, { rating, comment }) => {
   try {
      const response = await axiosInstance.put(`/api/orders/${orderId}/review`, { rating, comment });
      return response.data;
   } catch (error) {
      throw error;
   }
